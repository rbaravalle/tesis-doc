...
\noindent entonces una cadena de derivaciones podría resultar $a \Rightarrow b \Rightarrow ab \Rightarrow bab \Rightarrow abbab \Rightarrow \dots$, donde las reglas aplicadas en cada paso serían $p1$, $p2$, $\{p1, p2\}$, $\{p2, p1, p2\}$.
...
\begin{align*}
B_{H}(0,w) &= b_{0},\\
B_{H}(u,w)- B_{H}(0,w) &= \frac{1}{\Gamma(H+0.5)} \Bigg\{ \int_{-\infty}^{u} [(u-s)^{H-0.5} - (-s)^{H-0.5} ] dB(s,w) + \\
&  \int_{0}^{u} (u-s)^{H-0.5} dB(s,w) \Bigg \},
\end{align*}


\noindent donde $b_{0}$ es un número real, $0 < H < 1$ es el coeficiente de Hurst, $-\infty < u < \infty $, $s$ la variable de integración temporal, y $w$ es un conjunto de valores aleatorios en un espacio uniforme de muestras.
Cuando $H = 0.5$ se obtiene movimiento Browniano convencional.

En el dominio frecuencial (al cual se puede acceder por medio de la transformada de Fourier), se observa que el fBm no posee una frecuencia dominante, es decir, la función está compuesta por un continuo de frecuencias, sin ninguna dominante.
Además, la contribución de cada frecuencia es inversamente proporcional a la misma, es decir que, para valores de H entre 0 y 0.5, el ruido fBm pertenece a la clase de ruidos $\frac{1}{f}$.
El ruido presenta autoafinidad estadística.
Intuitivamente significa que las características estadísticas del ruido son similares si se aplica una combinación de escalas adecuada a la amplitud y al tiempo (determinada por H).
...
Cuando $H = 0$, la función generada es densa a todas las escalas, por lo que topológicamente es indistinguible de un plano (la probabilidad de que la función pase por un disco de radio arbitrariamente pequeño es igual a uno).
Cuando $H = 1$, la función es rectificable (aproximable con precisión arbitraria por una poligonal) y por lo tanto topológicamente indistinguible de un segmento de recta.
...
Trabajos previos a la aparición de la ecuación del renderizado alcanzaron resultados parciales por medio de diversos métodos ad-hoc, sin lograr sintetizar un único método que englobe todos los fenómenos en investigación hasta ese entonces.
Los más emblemáticos han sido el trazado de rayos \cite{Whitted1980} y la radiosidad \cite{Goral1984}.
%Entre los casos particulares de la ecuación del renderizado,  los cuales intentaron capturar diferentes comportamientos de las BRDFs.
...
Luego de propuesta la ecuación, se observó que ambos métodos eran casos particulares de la misma.
Se pueden consultar más detalles de la ecuación en \cite{Kajiya1986}.
...
Gracias a esta expresión se puede calcular una cantidad de interés, llamada {\em Transmitancia}, $T_{r}$, la cual describe la proporción de fotones que pueden viajar sin obstrucciones entre dos puntos en un medio (en línea recta).
...
Una simplificación muy común de esta función consiste en ignorar los fenómenos anteriores y limitar la definición de la BRDF al par de ángulos de entrada y de salida, para cada punto de la superficie, como se muestra en la Fig. \ref{fg:brdf}.
...
Para esto, se modelaron las anisotropías en la reflectancia presentes en las superficies (reflectancia diferente para distintos ángulos), otorgando un mayor realismo.
...
Finalmente, se ha intentado representar los datos medidos en materiales reales con modelos matemáticos, buscando reducir costos de almacenamiento \cite{Ngan2005}.
...
\noindent donde $E_{i}$ constituye la {\em irradiancia} entrante, definida como flujo incidente sobre unidad de área, y $L_{r}$ nota la radiancia reflejada (los ángulos coinciden con los de la Fig.~\ref{fg:brdf}).
...
Por ejemplo, las contribuciones especulares (reflejos perfectos) y difusa (la luz se refleja en todas las direcciones de manera equitativa).
...
Estos poros microscópicos son aún más pequeños que los poros presentes en la piel humana, resultando invisibles al ojo humano.
%Si bien su tamaño excede la longitud de onda de la luz.
...
Para obtener una distribución estadística de las geometrías de los poros presentes en una superficie, se deben modelar los mismos.
El modelado de los poros se lleva a cabo por medio de simulaciones estocásticas Monte Carlo sobre posibles geometrías microscópicas de poros bajo ciertas suposiciones.
...
Existen dos tipos principales de pelo: el {\em vello}, presente en prácticamente toda la superficie del cuerpo, con $1mm$ de alto, y el {pelo terminal}, presente únicamente en el cuero cabelludo.
