...
Las imágenes generadas utilizando los métodos de los capítulos previos provocan que los seres humanos perciban determinados materiales en escenas renderizadas.
...
En la Figura~\ref{fig:bread} se observa una imagen de cada tipo de pan (primera fila) y su binarización correspondiente utilizando el algoritmo descripto (segunda fila).
Se removieron de la imagen elementos pequeños de $1$ o $2$ píxeles utilizando una operación de {\em opening} (erosión seguida de dilatación, ver capítulo anterior) con un elemento estructurante de tamaño $2\times 2$ píxeles. Este método presenta buenos resultados incluso con condiciones de iluminación variando sobre la imagen.
...
Es decir, dentro de la imagen se presenta más de una DF, dependiendo del tamaño de las burbujas en consideración, o del tamaño de la ventana donde se cuenta la cantidad de píxeles.
...
En esta sección proponemos el empleo del Espectro Multifractal (Multifractal Spectrum en inglés, MFS) para describir y clasificar diferentes migas de panes.
...
Se comienza computando $\alpha(x)$ para cada píxel $x$ de la imagen.
